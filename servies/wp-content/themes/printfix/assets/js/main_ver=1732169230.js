(function($) {
   ...
   ========================================*/
    ...
    /*feedback__active-2***/
    var feedbacktwo = new Swiper(".postbox__thumb-slider-active", {
        slidesPerView: 1,
        spaceBetween: 30,
        loop: true,
        autoplay: {
    ...
    function RRCountdown() {
        $('#getting-started').countdown($('#getting-started').data('due-time').replace(/-/g, '/'), function(event) {
            $(this).html(event.strftime(' <div><span>%D</span></div>  <div><span>%H</span></div> <div><span>%M</span></div> <div><span>%S</span></div>'));
        });
    }

    $(window).on('elementor/frontend/init', function() {
        elementorFrontend.hooks.addAction('frontend/element_ready/rr_countdown.default', RRCountdown);
    });

    // Nice Select Js
    $('.sidebar__widget select, .footer__widget select', ).niceSelect();
